...
(() => {
    ...
    function runPartActions(actions, element) {
        if (actions.trigger) {
            if (actions.trigger[0] === '{') {
                let triggers = htmx.__HCON.parse(actions.trigger);
                for (let name in triggers) {
                    let detail = triggers[name];
                    let target = detail?.target ? htmx.find(detail.target) : element;
                    htmx.trigger(target, name, typeof detail === 'object' ? detail : {value: detail});
                }
            } else {
                for (let name of actions.trigger.split(',')) htmx.trigger(element, name.trim(), {});
            }
        }
        ...
        if (actions.redirect) {
            location.href = actions.redirect;
            return true;
        }
        if (actions.location) {
            let path = actions.location;
            let options = {};
            if (path[0] === '{' || /[\s,]/.test(path)) {
                options = htmx.__HCON.parse(path);
                path = options.path;
                delete options.path;
            }
            if (options.replace == null) options.push ??= 'true';
            htmx.ajax('GET', path, options);
            return true;
        }
        return false;
    }
    ...
    async function handleMultipartResponse(ctx, type) {
        ...
        let defaultTarget = envelopeRetarget ?? envelopeTarget ?? ctx.target;
        let defaultSwap = envelopeReswap ?? envelopeSwap ?? ctx.swap;
        let defaultSelect = envelopeReselect ?? envelopeSelect ?? ctx.select;
            ...
            while (element.isConnected && !connection.cancelled) {
                ...
                try {
                    while (true) {
                        ...
                        let handling = (async () => {
                            if (actions.pushUrl == null && actions.push != null) actions.pushUrl = actions.push;
                            if (!runPartActions(actions, ctx.sourceElement)) {
                                let hx = {};
                                for (let key in actions) hx[key.toLowerCase()] = actions[key];
                                await htmx.swap({
                                    ...ctx,
                                    text,
                                    target,
                                    swap: swap ?? defaultSwap,
                                    select,
                                    push: false,
                                    replace: false,
                                    hx
