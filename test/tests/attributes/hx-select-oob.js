describe('hx-select-oob', function() {
    ...
    it('basic hx-select-oob works', async function () {
        ...
        let div = initHTML('<div id="target" hx-get="/test" hx-select="#d1" hx-select-oob="#d2" hx-swap="innerHTML">Click</div><div id="d2"></div>')
    ...
    it('multiple hx-select-oobs works', async function () {
        ...
        let div = initHTML('<div id="target" hx-get="/test" hx-select="#d1" hx-select-oob="#d2, #d3" hx-swap="innerHTML">Click</div><div id="d2"></div><div id="d3"></div>')
        await clickAndWait(div)
        assertTextContentIs('#target', 'foo')
        assertTextContentIs('#d2', 'bar')
    ...
    it('basic hx-select-oob ignores bad selector', async function () {
        mockResponse('GET', '/test', '<div id="d1">foo</div><div id="d2">bar</div>')
        let div = initHTML('<div id="target" hx-get="/test" hx-select="#d1" hx-select-oob="#bad" hx-swap="innerHTML">Click</div><div id="d2"></div>')
        await clickAndWait(div)
        assertTextContentIs('#target', 'foo')
    ...
    it('hx-select-oob works with advanced swap style like strip:false and target:', async function () {
        mockResponse('GET', '/test', '<div id="d1">foo</div><div id="d2">bar</div>')
        initHTML('<div id="d0" hx-get="/test" hx-select="#d1" hx-select-oob="#d2:afterend strip:false target:#d0" hx-swap="innerHTML">Click</div>')
    ...
    it('hx-select-oob works with basic targeting selector', async function () {
        mockResponse('GET', '/test', '<div id="d1">foo</div><div id="d2">bar</div>')
        let div = initHTML('<div id="target" hx-get="/test" hx-select="#d1" hx-select-oob="#d2:outerHTML:#d3" hx-swap="innerHTML">Click</div><div class="foo" id="d3"></div>')
        await clickAndWait(div)
        assertTextContentIs('#target', 'foo')
        assertTextContentIs('#d2', 'bar')
    ...
    it('basic hx-select-oob works supports non text based selectors', async function () {
        mockResponse('GET', '/test', '<div id="d1">foo</div><div class="foo" id="d2">bar</div>')
        let div = initHTML('<div id="target" hx-get="/test" hx-select="#d1" hx-select-oob=".foo" hx-swap="innerHTML">Click</div><div id="d2"></div>')
        await clickAndWait(div)
        assertTextContentIs('#target', 'foo')
        let div2 = document.getElementById('d2')
        assert.equal(div2.textContent, 'bar')
    ...
    it('basic hx-select-oob works with CSS escaped id containing "."', async function () {
        ...
        let div = initHTML('<div id="target" hx-get="/test" hx-select="#d1" hx-select-oob="#my\\.div3" hx-swap="innerHTML">Click</div><div id="my.div3"></div>')
        await clickAndWait(div)
        assertTextContentIs('#target', 'foo')
    ...
    it('hx-select-oob can select multiple elements with a selector', async function () {
        ...
        let div = initHTML('<div id="target" hx-get="/test" hx-select="#d1" hx-select-oob=".foo" hx-swap="innerHTML">Click</div><div id="d2"></div><div id="d3"></div>')
        await clickAndWait(div)
        assertTextContentIs('#target', 'foo')
        let div2 = document.getElementById('d2')
        assert.equal(div2.textContent, 'bar')
