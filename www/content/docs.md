...
Download `htmx.min.js` [from jsDelivr](https://cdn.jsdelivr.net/npm/htmx.org@2.0.11/dist/htmx.min.js) and add it to the appropriate directory in your project
...
| `htmx.config.reportValidityOfForms`    | defaults to `false`, whether to report form validation errors to the user and focus the first invalid input, matching native form submission (htmx 4 validates forms by default; see `hx-validate`)                                                                                                                     |
</div>
