...
* The [`hx-swap`](/reference/attributes/hx-swap) attribute now supports two new modifiers:
...
* The [`hx-push-url`](/reference/attributes/hx-push-url) attribute now can optionally take a URL to push, in addition to `true` and `false`
* Added the [`hx-vars`](/reference/attributes/hx-vars) attribute that allows you to dynamically add to the parameters that will be submitted with a request
