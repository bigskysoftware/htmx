...
description: "Release announcement, 2021-07-12."
created: 2021-07-12
authors: ["htmx"]
---
...
* Added the [hx-request](/reference/attributes/hx-request) attribute, allowing you to configure the following aspects of the request
...
* Conditional polling via the [hx-trigger](/reference/attributes/hx-trigger) attribute
* `document` is now a valid pseudo-selector on the [hx-trigger](/reference/attributes/hx-trigger) `from:` argument, allowing you
  to listen for events on the document.
* Both the `scroll` and `show` options on [hx-swap](/reference/attributes/hx-swap) now support extended syntax for selecting the
