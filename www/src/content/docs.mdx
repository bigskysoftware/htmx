...
- `detail.ctx.swap.content` (response body, modifiable in `htmx_after_request`)
...
Removed. Modify `detail.ctx.swap.content` in `htmx_after_request`:
...
Event flow: response received, `ctx.swap.content` set, `htmx:after:request` fires, content consumed into a fragment, `htmx:before:swap`.
...
| `transformResponse(text, xhr, elt)`     | `htmx_after_request` hook (modify `detail.ctx.swap.content`)  |
