...
| [`morphPreserve`](/reference/config/htmx-config-morphPreserve)                       | `null` | CSS selector. Matching elements kept entirely (attrs + children)                                              |
| [`morphPreserveChildrenOf`](/reference/config/htmx-config-morphPreserveChildrenOf)       | `null` | CSS selector. Matching elements: children kept, own attrs still change                                         |
| [`morphPreserveAttributes`](/reference/config/htmx-config-morphPreserveAttributes)   | `null` | Attribute name patterns (`*` wildcard, `(a\|b)` alternation). Matching names kept on any element, rest still changes |
...
Three config keys tell morph what to leave alone, each for a different scope:

```javascript
htmx.config.morphPreserve            = ".widget, .third-party";                            // matching elements: kept entirely (attrs + children)
htmx.config.morphPreserveChildrenOf  = "lit-component, .sortable";                          // matching elements: children kept, own attrs still change
htmx.config.morphPreserveAttributes  = ["data-state-*", "aria-(label|checked)", "style"];   // any element: matching attribute names kept, rest still changes
```

See [`morphPreserve`](/reference/config/htmx-config-morphPreserve), [`morphPreserveChildrenOf`](/reference/config/htmx-config-morphPreserveChildrenOf), and [`morphPreserveAttributes`](/reference/config/htmx-config-morphPreserveAttributes) for full reference.
...
The one area to be aware of is morph swaps when used alongside JS frameworks like Alpine that set `style` attributes via JavaScript. During morph, htmx reconciles attributes between the old and new element via `setAttribute`, including any `style` value. Under a strict `style-src` policy without `'unsafe-inline'`, this `setAttribute("style", ...)` call will produce a CSP violation.

Add `"style"` to [`morphPreserveAttributes`](/reference/config/htmx-config-morphPreserveAttributes) to skip it:

```html
<meta name="htmx-config" content='{"morphPreserveAttributes":["style"]}'>
```
...
| `htmx.config.morphPreserve`           | CSS selector, defaults to `null`. Matching elements kept entirely (attrs + children) |
| `htmx.config.morphPreserveChildrenOf`   | CSS selector, defaults to `null`. Matching elements: children kept, own attrs still change |
| `htmx.config.morphPreserveAttributes` | Attribute name patterns, defaults to `null`. Matching names kept on any element, rest still changes. Accepts a comma- or whitespace-separated string OR an array. Patterns support `*` wildcard and `(a\|b)` alternation. |
