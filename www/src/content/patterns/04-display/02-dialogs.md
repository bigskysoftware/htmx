...
The simplest approach uses the [`hx-confirm`](/reference/attributes/hx-confirm) attribute to trigger native browser dialogs. These are triggered by the user interaction that would trigger the AJAX request, but the request is only sent if the dialog is accepted.
