...
| `htmx.config.prefix`              | defaults to `""` (empty string), allows you to use a custom prefix for htmx attributes (e.g., `"data-hx-"` to use `data-hx-get` instead of `hx-get`). Multiple prefixes can be specified as a comma-separated list (e.g., `"hx-,data-hx-"`). **Must be set via meta tag** — setting this after page load via JavaScript will not apply correctly as elements will already have been processed. |
