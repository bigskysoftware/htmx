...
And, with htmx, it can also be [very smooth](/patterns/records/edit-in-place), and not just constrained
to the simple list view/detail view approach many server side applications take.
...
disjoint parts of the UI cleanly? htmx offers [a few techniques for making this work](/patterns/advanced/update-other-content),
...
the techniques [mentioned above](/patterns/advanced/update-other-content) aren't satisfactory, then it may be
...
(Note, however, that for many applications, the "editable row" pattern is an
...
demonstrated
in the [drag-and-drop Sortable.js + htmx](/patterns/records/drag-to-reorder) example.
