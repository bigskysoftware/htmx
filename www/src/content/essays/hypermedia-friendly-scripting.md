...
In [Hypermedia-Driven Applications](/essays/hypermedia-driven-applications) we discuss how to build
web applications in such a manner that they are _hypermedia_-driven, in contrast with the popular SPA approach, in which
they are _JavaScript_ and, at the network-level, [RPC-driven](/essays/how-did-rest-come-to-mean-the-opposite-of-rest).
...
The prime directive of an HDA is to use [Hypermedia As The Engine of Application State](/essays/hateoas).
...
not part of the standard hypermedia (HTML) tool set, but do so in a way that plays well with HTML, rather than
relegating
...
there are simpler cases where client-side state are warranted and perfectly consistent with a Hypermedia-Driven
Application.
...
A good example of this is the [Sortable.js](/patterns/records/drag-to-reorder) example, in which htmx listens for
...
[`hx-trigger`](/reference/attributes/hx-trigger) attribute and then issues an HTTP request, exchanging hypermedia with
the
...
outside of the normal hypermedia-exchange mechanism is necessary, the most hypermedia-friendly approach is to use the
island
...
listed here, and we consider it an "optional" rule for hypermedia-friendly scripting: worth considering but not
required.
...
That being said, the inline scripting and the JSX approach do share an advantage in common: both satisfy
the [Locality of Behavior(LoB)](/essays/locality-of-behaviour),
...
Using techniques like invoking library functions or
using [hyperscript behaviors](https://hyperscript.org/features/behavior/)
