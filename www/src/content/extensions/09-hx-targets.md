...
Add an `hx-targets` attribute with a CSS selector to any element that makes an htmx request. The response will be swapped into every matching element, using the element's normal [`hx-swap`](/reference/attributes/hx-swap) strategy.
...
The `hx-targets` attribute supports htmx's extended selector syntax, including `closest`, `find`, `next`, and `previous`:
...
When `hx-targets` is present, it overrides [`hx-target`](/reference/attributes/hx-target). The swap style from [`hx-swap`](/reference/attributes/hx-swap) (default: `innerHTML`) is applied to each matched element.
...
- Each target receives a clone of the full response. To distribute different parts of the response to different targets, use [`<hx-partial>`](/docs#partials-hx-partial) or [`hx-swap-oob`](/reference/attributes/hx-swap-oob) instead.
