...
<script src="https://cdn.jsdelivr.net/npm/htmx.org@__VERSION__/dist/ext/hx-multipart.min.js"></script>
```
...
Any other `HX-*` header becomes a [custom action](/reference/events/htmx-before-actions#actions).
...
`reason` is `part`, `removed`, `cancelled`, `ended`, or `cleanup`.
...
Part actions also fire [`htmx:before:actions`](/reference/events/htmx-before-actions) and [`htmx:after:actions`](/reference/events/htmx-after-actions). Their detail includes `ctx` and `part`.
