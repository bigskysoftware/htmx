...
import nestingExample from '../../assets/img/nesting-example.png';
...
_-Roy Fielding, https://www.ics.uci.edu/~fielding/pubs/dissertation/rest_arch_style.htm#sec_5_1_5_
...
With [htmx](/) and the additional UX possibilities that it gives you, we believe that many modern web applications
can be built
...
we will give you some ways to think about if hypermedia will be a good fit for an application or feature you are
building.
...
In [The Mother Of All htmx Demos](/essays/a-real-world-react-to-htmx-port), David Guillot of Contexte shows how
replacing
...
Another area where hypermedia has a long track-record of success
is [CRUD](https://en.wikipedia.org/wiki/Create,_read,_update_and_delete)-y
...
And, with htmx, it can also be [very smooth](/patterns/edit-in-place), and not just constrained
to the simple [list view/detail view](/patterns/edit-row) approach many server side applications take.
...
areas of the screen. A good example of this, and one that often comes up when discussing the hypermedia approach, is the
issue
...
disjoint parts of the UI cleanly? htmx offers [a few techniques for making this work](/patterns/update-other-content),
...
<Image src={nestingExample} alt="Nested UI example" />
...
to
[target](/reference/attributes/hx-target) just that particular area for update when a modification is made to the respective
...
changing
our API to satisfy the new requirements. This is a [unique aspect](/essays/hateoas) of developing with
hypermedia, and we [discuss it in more detail here](/essays/hypermedia-apis-vs-data-apis).

Of course, there may be UI requirements that do not allow for grouping of dependent element in this manner and, if
the techniques [mentioned above](/patterns/update-other-content) aren't satisfactory, then it may be
...
(Note, however, that for many applications, the ["editable row"](/patterns/edit-row) pattern is an
...
via [events](https://developer.mozilla.org/en-US/docs/Learn/JavaScript/Building_blocks/Events), in the manner
demonstrated
in the [drag-and-drop Sortable.js + htmx](/patterns/drag-to-reorder) example.
...
in favor in web development. Many companies have adopted React as their standard library for building web
applications.  
...
on every
cell update. Google Maps, on the other hand, responds rapidly to mouse movements and simply can't afford a server round
trip for
...
developer, even if it doesn't become your favorite hammer. There is a good [theoretical basis](/essays/hateoas)
for the approach, [practical benefits for many applications](/essays/a-real-world-react-to-htmx-port),
