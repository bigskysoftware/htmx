...
description: "In this article, Carson Gross explores how the term REST (Representational State Transfer) evolved to mean nearly the opposite of its original definition in modern web development. It traces how REST, originally defined by Roy Fielding to describe the web's architecture of hypermedia-driven interactions, came to be widely misused as a term for JSON-based APIs that lack the key hypermedia constraints that define true REST architectural style."
created: 2022-07-18
modified: 2022-11-26
...
When you see a job post mentioning REST or a company
discussing [REST Guidelines](https://github.com/microsoft/api-guidelines/blob/vNext/Guidelines)
...
In this post, I'd like to give you
a [brief, incomplete and mostly wrong](https://james-iry.blogspot.com/2009/05/brief-incomplete-and-mostly-wrong.html)
...
This is known as
the [uniform interface constraint](https://en.wikipedia.org/wiki/Representational_state_transfer#Uniform_interface),
and more specifically within that concept, the idea
of [Hypermedia As The Engine of Application State (HATEOAS)](https://htmx.org/essays/hateoas/)
...
While all this was happening, another important technology was in the process of being
born: [JSON](https://www.json.org/json-en.html)
...
In 2008, Martin Fowler published an article popularizing
...
* [Hypermedia Driven Applications](/essays/hypermedia-driven-applications)
...
that REST found in the general, human-oriented
...
*entire
networking architecture* of SPA applications moved over to the JSON RPC style. Additionally, due to the complexity of
these
...
Even though they were doing things like publishing swagger documentation for their RESTful APIs
or [complaining about API
...
Finally, in the late 2010s, people had had enough:  REST, even in its RESTless form, simply wasn't keep up with the
needs
...
[People sense something is wrong](/essays/a-response-to-rich-harris), and maybe REST, real, actual REST,
not RESTless, could be a part of [the answer to that](/essays/spa-alternative).
